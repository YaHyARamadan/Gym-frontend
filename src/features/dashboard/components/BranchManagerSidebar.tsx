...
const NAV_ITEMS = [
  { href: "/dashboard/branch-manager", label: "الرئيسية", icon: LayoutGrid },
  { href: "/dashboard/branch-manager/staff", label: "الموظفون", icon: Users },
  { href: "/dashboard/branch-manager/members", label: "الأعضاء", icon: UserCheck },
  { href: "/dashboard/branch-manager/subscriptions", label: "الاشتراكات", icon: CreditCard },
  { href: "/dashboard/branch-manager/finance", label: "المدفوعات والمصاريف", icon: Banknote },
  { href: "/dashboard/branch-manager/reports", label: "التقارير", icon: BarChart3 },
  { href: "/dashboard/branch-manager/complaints", label: "الشكاوى", icon: AlertCircle },
  { href: "/dashboard/branch-manager/lockers", label: "الخزائن", icon: Key },
  { href: "/dashboard/branch-manager/store", label: "المتجر والمخزون", icon: ShoppingBag },
  { href: "/dashboard/branch-manager/notifications", label: "الإشعارات", icon: Bell },
];
      ...
      <div className="flex flex-col h-full py-6 px-4">
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto pr-1">
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            const active =
              pathname === href || (href !== "/dashboard/branch-manager" && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
