...
const NAV_ITEMS = [
  { href: "/dashboard/reception", label: "الرئيسية", icon: LayoutGrid },
  { href: "/dashboard/reception/attendance", label: "تسجيل الحضور", icon: UserCheck },
  { href: "/dashboard/reception/members", label: "الأعضاء", icon: Users },
  { href: "/dashboard/reception/payments", label: "عملياتي المالية", icon: CreditCard },
  { href: "/dashboard/reception/store", label: "المتجر", icon: ShoppingBag },
  { href: "/dashboard/reception/complaints", label: "الشكاوى", icon: AlertCircle },
  { href: "/dashboard/reception/lockers", label: "الخزائن", icon: Key },
];
      ...
      <div className="flex flex-col h-full py-6 px-4">
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto pr-1">
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            const active =
              pathname === href || (href !== "/dashboard/reception" && pathname.startsWith(href));
            return (
              <Link
                key={href}
                href={href}
